...
class AuthController {
    static async register(req, res) {
        try {
            ...
            const exists = await prisma_1.prisma.user.findFirst({ where: { OR: [{ email }, { cpf }] } });
            if (exists)
    ...
    static async me(req, res) {
        ...
        const user = await prisma_1.prisma.user.findUnique({ where: { id }, select: { id: true, name: true, email: true, cpf: true, phone: true } });
        res.json(user);
    }
}
