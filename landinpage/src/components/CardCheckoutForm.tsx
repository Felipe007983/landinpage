...
export function CardCheckoutForm({ amount, orderId, mpPublicKey, onPaymentSuccess, onCancel }: CardCheckoutFormProps) {
    ...
    return (
        <form id="form-checkout" className="w-full flex flex-col gap-4 text-left">
            <h3 className="text-xl font-bold uppercase tracking-widest text-amber-500 mb-2">Cartão de Crédito</h3>
            ...
            <div className="flex flex-col gap-1">
                ...
                <input type="text" id="form-checkout__cardholderName" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors placeholder-zinc-500" placeholder="Nome como no cartão" />
            </div>

            <div className="flex flex-col gap-1">
                <label className="text-sm text-gray-400">E-mail do Titular</label>
                <input type="email" id="form-checkout__cardholderEmail" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors placeholder-zinc-500" placeholder="E-mail" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                    <label className="text-sm text-gray-400">Banco Emissor</label>
                    <select id="form-checkout__issuer" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors"></select>
                </div>
                <div className="flex flex-col gap-1">
                    <label className="text-sm text-gray-400">Parcelas</label>
                    <select id="form-checkout__installments" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors"></select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-[100px_1fr] gap-4">
                <div className="flex flex-col gap-1">
                    ...
                    <select id="form-checkout__identificationType" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors"></select>
                </div>
                <div className="flex flex-col gap-1">
                    <label className="text-sm text-gray-400">Número do Documento</label>
                    <input type="text" id="form-checkout__identificationNumber" className="h-10 px-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-amber-500 transition-colors placeholder-zinc-500" placeholder="000.000.000-00" />
                </div>
            </div>

            {progress > 0 && (
                <div className="w-full bg-zinc-800 rounded-full h-1 mt-2 overflow-hidden">
                    <div className="bg-amber-500 h-1 transition-all duration-300" style={{ width: `${progress}%` }}></div>
                </div>
            )}
            ...
            <div className="flex gap-4 mt-4">
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex-1 py-3 bg-amber-500 text-black rounded-lg font-bold uppercase hover:bg-amber-400 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                    {loading ? (
                        <>
